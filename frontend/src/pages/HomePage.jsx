...
export default function HomePage({ searchTerm }) { // Recebe searchTerm como prop
  ...
  const handleDelete = async (e, pollId) => {
    // ... (lógica de deletar continua a mesma)
    e.stopPropagation(); e.preventDefault();
    if (window.confirm('Tem certeza?')) {
      try { await api.delete(`/polls/${pollId}`); setPolls(polls.filter(p => p._id !== pollId)); }
      catch (error) { alert('Falha ao excluir a enquete.'); }
    }
  };

  // Filtra as enquetes com base no searchTerm
      ...
      <div className={styles.pollsContainer}>
        ...
        {filteredPolls.length > 0 ? (
          <div className={styles.pollList}>
            {filteredPolls.map(poll => { // Mapeia sobre 'filteredPolls'
              ...
              return (
                <Link key={poll._id} to={`/poll/${poll._id}`} className={styles.pollItem}>
                  <div className={styles.pollItemContent}>
                    ...
                    {canDelete && (
                      <button onClick={(e) => handleDelete(e, poll._id)} className={styles.deleteButton}>Excluir</button>
                    )}
                  </div>
                </Link>
              );
